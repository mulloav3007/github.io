...
% L_GDP_GAP = b1*L_GDP_GAP{-1} - b2*(TPM-TPMN1) + b3*(CRECSC-ss_CRECSC) + b4*(FFR{-1}-ss_FFR) +...
%            + b5*(VIX{-1}-ss_VIX) + b6*(UST10{-1}-ss_UST10) + b7*(L_PCU-ss_L_PCU) + ...
%            + b8*(L_WTI-ss_L_WTI) + b9*(T_COLOC-ss_T_COLOC) + SHK_L_GDP_GAP;

%output persistence;
%b1 varicaes between 0.1 (extremely flexible) and 0.95(extremely persistent)
%p.b1 = 0.7;
p.b1 = 0.5;
...
p.b5 = 0.01;
p.b6 = 0.104;
p.b7 = 1.501/100;   %0.01501
p.b8 = -1.066/100;  %-0.01066
p.b9 = -0.184;   
...
% CHECK POINT ESTIMATES WITH PABLO GARCIA
% DLA_CPIXFE =  a1*DLA_CPIXFE{-1} + (1-a1)*DLA_CPIXFE{1} + a2*L_GDP_GAP{-1} + a3*(L_Z_GAP) + a4*Q1 + a5*Q2 + a6*Q3 + a7*Q4 + SHK_DLA_CPIXFE;
...
%% A. The standard rule
% TPM = g1*TPM{-1} + (1-g1)*(TPMN1 + g2*(D4L_CPI{+3} - ss_D4L_CPI_TAR) + g3*L_GDP_GAP) + SHK_RS;
% TPM = g1*TPM{-1} + (1-g1)*TPMN1 + (1-g1)*g2 *(D4L_CPI{+3} - ss_D4L_CPI_TAR) + (1-g1)*g3 *L_GDP_GAP + SHK_RS;
...
p.ss_L_Z = 100*log(100); 

% Potential output growth
p.ss_DLA_GDP_BAR = 2.0; %1
...
p.ss_VIX = 20;
p.ss_UST10 = 4.0;
p.ss_L_PCU = 100*log(3.5);
p.ss_L_WTI = 100*log(75);
